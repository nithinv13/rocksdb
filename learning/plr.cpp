...
point get_unshared_point(std::string& key, uint64_t offset, uint32_t minn_shared) {
    ...
    if (key.size() == minn_shared) x = 0.0f;
        ...
        std::string x_str = key.substr(minn_shared, minn_shared + key_size_changer);
...
std::vector<Segment> SLSR::train(std::vector<std::pair<std::string, key_type> >& keys, long double break_off_cost) {
	...
	for (j = 1; j <= size; j++)	{
		long double key = stoll(keys[j-1].first.substr(0, key_size_changer));
		...
		for (i = 1; i <= j; i++) {
			...
			intercept[i][j] = (y_sum - slope[i][j] * x_sum) / double(interval);
           	...
           	for (k = i, error[i][j] = 0.0, max_error[i][j] = 0.0; k <= j; k++)	{
				long double pt_err = (long double)keys[k-1].second - slope[i][j] * stoll(keys[k-1].first.substr(0, key_size_changer)) - intercept[i][j];
...
std::vector<Segment> SimLR::train(std::vector<std::pair<std::string, key_type> >& keys, std::vector<double>& simLR_bounds) {
	
	size = keys.size();
	long double mean_x = 0, mean_y = 0, cum_xy = 0, cum_x2 = 0;
	for (int i = 0; i < size; i++) {
		long double key = stoll(keys[i].first.substr(0, key_size_changer));
		mean_x += key;

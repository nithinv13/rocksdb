...
// Current the following isn't equivalent to OS_LINUX.
 ...
 public:
  ...
  void Run() {
    ...
    while (std::getline(benchmark_stream, name, ',')) {
      ...
      } else if (name == "fillrandomcustom2") {
        fresh_db = true;
        method = &Benchmark::FillRandomCustom2;
 ...
 private:
  ...
  void FillRandomCustom2(ThreadState* thread) {
    ...
    DB* db = SelectDB(thread);
    db->GetOptions().rate_limiter->SetBytesPerSecond(FLAGS_rate_limiter_bytes_per_sec);
    for (int i = 0; i < FLAGS_num*iterations; i++) {
      int val_size = FLAGS_value_size;
      std::unique_ptr<const char[]> key_guard;
      Slice key = AllocateKey(&key_guard);
      std::string a;
    ...
    for (int i = 0; i < iterations; i++) {
      duration = Duration(FLAGS_duration, 2*readwrites_);
      while (!duration.Done(1)) {
        int op_prob = thread->rand.Next() % 100;
        if (op_prob < 40) {
          int val_size = FLAGS_value_size;
    ...
    while (!duration.Done(1)) {
        ...
        if (thread->tid == 0){
            ...
            if (FLAGS_dynamic_compaction_rate && 
                ...
                printf("Adjust-compaction-rate; current-client-bandwidth: %ld ops/s; Bandwidth-taken: %ld MB/s; Left-for-compaction: %ld\n", 
    ...
    while (!duration.Done(entries_per_batch_)) {
      ...
      for (int64_t j = 0; j < entries_per_batch_; j++) {
        ...
        if (use_blob_db_) {
          ...
          // printf("In writerandom and the key is %s and val is %s\n", key.data(), val.data());
          batch.Put(key, val);
        } else {
